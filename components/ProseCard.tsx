  ...
  return (
    <article className="glass rounded-[28px] border border-white/10 p-6">
      <div className="mb-4 flex items-center justify-between gap-4">
        <span className="rounded-full border border-amber-400/20 bg-amber-500/10 px-3 py-1 text-xs uppercase tracking-[0.18em] text-amber-200">
          Проза
        </span>
        <span className="text-xs text-gray-500">{work.readingTime} мин</span>
      </div>
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="text-xs text-gray-500">
          👁 {work.views || 0} · ❤ {work.likes || 0}
        </div>

        <Link
          href={publicHref}
          className="rounded-2xl border border-amber-400/20 bg-amber-500/10 px-4 py-2.5 text-sm font-medium text-amber-200 hover:bg-amber-500/15"
        >
          Читать
        </Link>
      </div>
